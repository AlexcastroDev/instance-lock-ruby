...
class BookUpsertWorker
  include Sidekiq::Worker

  def perform(id)
    book = Book.find(id)

    unless book.upsert_author(async: false)
      #
    end
  end
end
...
class Book < ActiveRecord::Base
  ...
  def update_name
    self.author.with_lock do
      self.author.name = "Updated 2"
      self.author.save!
    end
  end

  def upsert_author(async: false)
    if async
      BookUpsertWorker.perform_async(author.id)
    else
      self.author.name = "Updated"
      self.author.save!
    end
  end
end
...
class BugTest < Minitest::Test
  def test_bug_locking_a_record_with_unpersisted
    ...
    book = author.books.create!(title: "Book 1", status: 0, price_cents: 1000)

    author.name = "Unpersisted Change"

    Sidekiq::Testing.inline! do
      BookUpsertWorker.perform_async(book.id)
    end
  end
end
